...
// 判断一个peer是否已经存在于unchoke_peers，存在返回1
...
/*改写，在返回下标,不在则返回-1 */  
/*
	int index = unchoke_peers.count - 1;
		
	for (; index >= 0; index--) {
		if( node == (unchoke_peers.unchkpeer[index] )
			return index;
	}
...
// 从unchoke_peers中获取下载速度最慢的peer的索引,如果找到更快的替换它
...
int get_rand_numbers(int length)
{
	...
	for(i = 0; i < piece_count; i++)  temp_num[i] = i;//等下rand_num 从这个数组里取
